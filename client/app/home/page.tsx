...
const ChatbotUI: React.FC = () => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const response = await fetch('https://visionsolve-1.onrender.com/api/ask', {
        method: 'POST',
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await fetch('https://visionsolve-1.onrender.com/api/upload/pdf', {
        method: 'POST',
        body: formData
      });
  ...
  const handleHandwrittenChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await fetch('https://visionsolve-1.onrender.com/api/upload/handwritten', {
        method: 'POST',
        body: formData
      });
            ...
            <div className={`rounded-xl p-3 w-fit max-w-[80%] ${
              ...
              {msg.type === "video" && msg.videoUrl && (
                <video controls className="mt-2 rounded-lg" style={{ maxWidth: '100%' }}>
                  <source src={`https://visionsolve-1.onrender.com${msg.videoUrl}`} type="video/mp4" />
